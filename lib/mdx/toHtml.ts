...
/**
 * Repair Windows-1252 UTF-8 mojibake: UTF-8 multi-byte sequences whose bytes
 * were stored/decoded as Windows-1252 characters (e.g. em dash — stored as â€").
 * Works by mapping Windows-1252 special chars back to their byte values then
 * re-decoding the resulting byte array as UTF-8. Leaves the string unchanged if
 * it is already valid Unicode or if decoding fails.
 */
function repairMojibake(str: string): string {
  // Windows-1252 code page: Unicode codepoint → byte value for the 0x80–0x9F range
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (cp <= 0x7F) {
      ...
      bytes.push(cp)
      if (cp >= 0x80) needsRepair = true
    } else if (w1252[cp] !== undefined) {
      bytes.push(w1252[cp])
      needsRepair = true
    } else {
      // Contains genuine Unicode outside W1252 — already correctly encoded
      return str
    }
  }
  if (!needsRepair) return str
  try {
